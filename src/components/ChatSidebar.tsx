...
export const ChatSidebar = () => {
	...
	return (
		<>
			<Dialog open={diaglogIsOpen} onOpenChange={setDiaglogIsOpen}>
				<DialogContent>
					<DialogHeader>
					...
					<div className="space-y-1">
						...
						<Input id="thread-title" value={textInput} onChange={(e) => setTextInput(e.target.value)} placeholder="Your new thread title" />
					</div>

					<DialogFooter>
						<Button variant="secondary" onClick={() => setDiaglogIsOpen(false)}>
							Cancel
						</Button>
			...
			<SidebarPrimitive>
				<SidebarHeader>
					<Button onClick={() => setDiaglogIsOpen(true)} className="w-full justify-start" variant="ghost">
				...
				<SidebarContent>
					<SidebarGroup>
						<SidebarGroupContent>
							<SidebarGroupLabel className="font-bold"># Recent Chats</SidebarGroupLabel>
							<SidebarMenu>
								{threads?.map((thread) => (
